...
export default function Sidebar() {
  ...
  const menuItems = [
    { name: "Dashboard", path: "/", icon: LayoutDashboard, iconSize: "size-[30px]" },
    { name: "Trend", path: "/proyeksi-tren", icon: TrendingUp, iconSize: "size-[24px]" },
    { name: "Early Warning", path: "/peringatan-dini", icon: AlertTriangle, iconSize: "size-[25px]" },
    { name: "Logistic", path: "/logistik", icon: Package, iconSize: "size-[25px]" },
    { name: "Settings", path: "/settings", icon: Settings, iconSize: "size-[25px]" },
    ...(isAdmin ? [{ name: "Admin Panel", path: "/admin", icon: ShieldCheck, iconSize: "size-[25px]" }] : []),
  ];
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                style={isActive ? {
                  backgroundImage: "linear-gradient(90deg, rgb(12, 129, 138) 0%, rgb(73, 153, 159) 27.943%, rgb(12, 129, 138) 63.703%, rgb(73, 153, 159) 93.817%)",
                } : undefined}
              >
                <Icon className={cn(
                  item.iconSize,
